...
if __name__ == "__main__":
    poolsize_list = os.listdir("out_files/")
    print(poolsize_list)
    ...
    for poolsize in poolsize_list:
        n_procs_list = os.listdir("out_files/" + poolsize)

        cputimes[poolsize] = {}

        for n_procs in n_procs_list:
            print(n_procs)
            files = os.listdir("out_files/" + poolsize + "/" + n_procs)

            cputimes[poolsize][n_procs] = np.zeros(len(files))

            for file_index, file in enumerate(files):
                with open("out_files/" + poolsize + "/" + n_procs + "/" + file, "r") as f:
                    searchlines = f.readlines()
                    cputimes[poolsize][n_procs][file_index] = qe_helper.search_times(searchlines, int(n_procs))[0]
