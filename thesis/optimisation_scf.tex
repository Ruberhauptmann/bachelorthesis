...
Instead, an estimate of the single core runtime is made by multiplying the runtime of the job on 4 cores by 4. \todo{die Annahme ist: der Job ist auf 1 Kern viermal so schnell wie auf 1 Kern, also multipliziere ich runtime(4 Kerne) mit 4 um ein estimate für single core perfomance zu bekommen}
...
In conclusion, systems with more electrons and by extension bigger matrices and longer iteration times seem to parallelize better and as such profit more from using more processors than systems with just a few electrons.
...
As discussed in sec. \ref{sec:parallel_computing}, the exact form of the speedup is not dependent on absolute times of parallelized and unparallelized parts of a calculations, but rather the proportion between these two (and can thus be characterized by just the purely serial part \(s\)).
...
\end{figure}
The runtime depicted in fig. \ref{fig:scaling_scf_nk_si_absolute} show that the choice between using \gls{openmpi} and \gls{oneapi} does not result in a big difference in total execution time, which follows the observations made in the benchmarks without \(k\)-point parallelization.
