...
\caption{Scalability for the Si benchmarking system. The speedup shows linear scaling with a slope of \(\nicefrac{1}{2}\) up to 10 processors, with worse than single core performance on more than on node. \emph{\QE 7.0 compiled with \gls{openmpi} 4.1.0, \texttt{nk 1} and \texttt{nd 1}}}
...
\caption{Scalability for the \TaS benchmarking system. The speedup is linear with a slope of one on a single node, with a drop in speedup over two nodes. \emph{\QE 7.0 compiled with \gls{openmpi} 4.1.0, \texttt{nk 1} and \texttt{nd 1}}}
