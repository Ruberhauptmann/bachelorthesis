...
\todo{density as calculated with KS states}
...
A natural choice, at least for delocalized (higher energy) orbitals is the discrete set of plane waves \(\ket{\vb{G}}\)
\begin{equation}
    \bra{\vb{r}} \ket{\vb{k} + \vb{G}} = \frac{1}{\sqrt{V}} e^{i (\vb{k} + \vb{G}) \vb{r}}
\end{equation}
where \(\vb{G}\) is a reciprocal lattice vector.
\todo{expansion of wave function in basis functions, also periodicity -> integrals over BZ}
One problem of this approach lies in the fact that the lower energy core electrons are in contrast very localized around the cores and as such need a lot of basis functions.
...
This fact can be used in computation by introducing a \emph{cutoff energy} \(E_{cutoff}\) which limits the number of basis functions by using only expansion coefficents with \(\vert \vb{k} + \vb{G} \vert \leq E_{cutoff}\).

\todo{not finished: kinetic energy -> fourier transform}
