...
\begin{itemize}
    ...
    \item \(\ope{U}_{\mathrm{e-e}}\) is the Coulomb interaction between the electrons and the nuclei
    \item \(\ope{V}_{\mathrm{n-e}}\) is the potential energy of the electrons in the field of the nuclei
...
\end{align}
The terms \(\ope{V}_{n-e}\) and \(\ope{W}_{n-n}\) here are just a function of the electronic coordinates \(\vb{r}\) and a constant respectively, so they can then be combined into an external potential \(\ope{V} (\vb{r})\) for the interacting electrons, so that the Hamiltonian reads
...
\begin{equation}
    T_{\mathrm{S}} [n(\vb{r})] = - \frac{1}{2} \sum_{i=1}^{N_e} \int \dd[3]{r} \Psi_i^* (\vb{r}) \Delta \Psi_i (\vb{r})\,.
\end{equation}
...
\todo{Something something good for computation, similaraly to hartree fock methods?}
...
In the framework of \acrshort{dft} a treatment of lattice vibrations can also be derived.
\todo{Maybe a bit more introduction?}
...
\end{equation}
The vibrational frequencies \(\omega\) are determined by
\begin{equation}
    \det \left\vert \frac{1}{\sqrt{M_{\alpha} M_{\beta}}} \pdv{E(\vb{R})}{\vb{R}_{\alpha}}{\vb{R}_{\beta}} - \omega^2 \right\vert = 0\,,
\end{equation}
so the calculation of the vibrational properties as well as the equilibrium geometry depend on the first and second derivatives of the energies \(E(\vb{R})\).
These derivatives can be calculated using the Hellman-Feynman theorem \ref{eq:dfpt_hellman_feynman}.
