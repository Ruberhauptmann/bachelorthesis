...
if __name__ == "__main__":
    ...
    nprocs_plots.plot(walltimes, n_procs, "si_ompi", "absolute", plot_error=True)

    nprocs_plots.plot(walltimes, n_procs, "small_si_ompi", "absolute", plot_error=True, figsize="small")
    ...
    nprocs_plots.plot(speedup, n_procs, "si_ompi", "speedup", plot_error=True)

    nprocs_plots.plot(speedup, n_procs, "small_si_ompi", "speedup", plot_error=True, figsize="small")

    ### Plot idle time

    wait_time = (walltimes - cputimes) / walltimes

    nprocs_plots.plot(wait_time, n_procs, "si_ompi", "wait", plot_error=False)

    ### Plot absolute and wait side by side

    nprocs_plots.plot_two([walltimes, wait_time], n_procs, "si_ompi", ["absolute", "wait"])

    ### Plot efficiency

    efficiency = speedup / n_procs

    nprocs_plots.plot(efficiency, n_procs, "si_ompi", "efficiency", plot_error=True)
